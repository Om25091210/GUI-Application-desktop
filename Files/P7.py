...
##...DONE...1:Common directory for images-----------logic------i wil use os module functions for taking a imput from user which will be a common directory and saving it in file.The place will be filled by a variable.only once it will execute using try except.i have to make try except little logical to execute one at a time
##...DONE..2:security question must be saved and should be asked in login canvas .Smooth functioning is of great importance.selection of 3 options out of 10 options 
##.........3:File security by file illusion method.Remember the file should be saved in the python files.
##...DONE..4:Think of the canvas that will carry wigdets i mean the design :)......
##.........5.A BUTTON TO SHOW INSTRUCTIONS
...
######################################################### MESSAGE BOX
path=os.path.dirname(os.path.realpath('P7'))
def Roar():
...
def mkw():
    wkm=m.showerror("ERROR!","WRONG USERNAME!")
...
root.iconphoto(True,PhotoImage(file="{}/newlogo.png".format(path)))      ######### here 'false' means this icon image applies only to this specific window but not future created toplevels but if we use 'TRUE' then it will be applied to all the root windows opening after it.
s=Canvas(root,bg="white",width=1366,height=768)
s.place(x=0,y=0)
png=PhotoImage(file="{}/bbg5.png".format(path))
s.create_image(685,0,anchor=N,image=png)
def pro(x=0):
    ...
    mc=Canvas(root1,bg="cyan",width=1366,height=768)
    mc.place(x=0,y=0)
    my=PhotoImage(file='{}/bbg2.png'.format(path))
    my11=my.subsample(2,3)
    mc.create_image(0,0, anchor = NW, image=my11)
    ...
    sc3=Canvas(mc,bg="red",width=136,height=137)
    sc3.place(x=607,y=130)
    picpz=PhotoImage(file="{}/newlogo.png".format(path))
    picpz1=picpz.subsample(10,10)
    sc3.create_image(70,7,anchor=N,image=picpz1)
    mc.create_line(450,100,650,100,fill="red")
    mc.create_line(600,100,950,100,fill="red")
    mc.create_line(450,303,910,100,fill="red")
    mc.create_line(910,303,450,100,fill="red")
    mc.create_line(450,303,910,303,fill="red")
    mc.create_line(450,200,910,200,fill="red")
    ...
    def snt(x=0):                 #####################################'cos' series of canvas and p_img of image:)
        ...
        cos.place(x=0,y=0)
        p_img=PhotoImage(file="{}/bg1.png".format(path))
        cos.create_image(680,0,anchor=N,image=p_img)
        p_img1=PhotoImage(file="{}/newlogo.png".format(path))
        pp_img1=p_img1.subsample(10,10)
        ...
        def e1(x=0):
            global use1
            ur1=StringVar()
            use1=Entry(cos,textvariable=ur1,bg="white",width=25,show='*')
            use1.place(x=640,y=300)
            use1.config(font=("Corbel",15))
            use1.focus()

        optionss=["om","pied","testing"]
        use=Combobox(cos,width=18,value=optionss)
        use.set("SELECT")
        use.place(x=640,y=220)
        use.config(font=("Algerian",18))

        u1=StringVar()
        us1=Entry(cos,textvariable=u1,bg="white",width=25)
        us1.place(x=640,y=300)
        ...
        def l1():
            k=open("plasma_T.txt",'r')
            m=k.readlines()
            o=0  
            q=len(m)
            print(q)
            for i in range(len(m)):             ## every lines is treated as element.so first line is i=0:
                print(i)
                for j in range(len(m[i])):      ## now,every lines alphabet is treated as elements.so i is traversing through each where i[0]=first alphabet:
                    try:
                        m1=use.get()            ## use is the entry variable.it is holding the name entered by the user.(username) and try block,because if user clicked the login button the .get() will generate an error.
                    except:
                        print()
                    m2=m1.upper()               ## The data saved in the file is in uppercase so the entered name must be in uppercase implicitly. 
                    x=m[i]                      ## The x will hold the whole line, equal to i=n.
                    z=x[6:]                     ## The z will hold the string which is the name of the user.it is slicing after 6: because the NAME:- is upto 5 index. 
                    z1=z.rstrip()               ## It Is of great importance because every username in the file is having a white space on the right while comparing you won't find any differnce but by this it works.
                    if z1==m2:                  ## HERE the name from the file and the entered name(from use) is compared.    
                       n=m[i+5]                 ## i+5 because the password line is at the 5th line from the NAME:- line.
                       try:
                           m3=use1.get()        ## use1.get() will bring the entered pssword from the login window.
                           m4=m3.upper()        ## The password is uppercased.
                       except:
                           print()
                       n1=n[10:]                ## SLICING from the 10th element.
                       n2=n1.rstrip()           ## again removing the white spaces from the right. 
                       if n2==m4:               ## error
                          o=1 
                          root2=Tk()
                          ...
                          break
                           ...
                           if str(q-1)==str(i):
                              print("ok")
                           if o==0 and 1==2:  ## error at the end of the whole checking (q-1==i) and to show the error only once in a loop (o==0) logic is used.
                              mkw()
                              o=2
                              break
        ...
        def l2():
            cos1=Canvas(root1,bg="black",width=1366,height=799)
            cos1.place(x=0,y=0)
            p_img=PhotoImage(file="{}/bg1.png".format(path))
            cos1.create_image(680,0,anchor=N,image=p_img)
            p_img1=PhotoImage(file="{}/newlogo.png".format(path))
            pp_img1=p_img1.subsample(2,2)
            cos1.create_image(700,50,anchor=N,image=pp_img1)
            
            cos1.create_line(500,200,500,500,fill="red")
            cos1.create_line(500,200,900,200,fill="red")
            cos1.create_line(900,200,900,500,fill="red")
            cos1.create_line(500,500,900,500,fill="red")
            imgg=PhotoImage(file="{}/bck.png".format(path))
            imgg1=imgg.subsample(5,5)

            
            ##################################################################PHONE NO. BLOCK
            ph_no_e=Entry(cos1,bg="white",width=21)
            ph_no_e.place(x=545,y=330)
            ph_no_e.config(font=("Helvetica",20))
            ph_no_e.focus()
            
            uss=Label(cos1,text=" PHONE NO. :",bg="black",fg="yellow",width=15)
            uss.place(x=575,y=250)
            uss.config(font=("Algerian",20))
            
            #################################################################BACK BUTTUUUUUUUUUNNN :)
            def bkc():
                cos1.destroy()
            kcab=Button(cos1,text="back",bg="white",width=55,command=bkc)
            kcab.place(x=510,y=435)
    ...
    def spt(x=0):
        ...
        cc=Canvas(dot,bg="white",width=136,height=137)
        cc.place(x=1170,y=10)

        picpz=PhotoImage(file="{}/newlogo.png".format(path))
        picpz1=picpz.subsample(10,10)
        cc.create_image(70,7,anchor=N,image=picpz1)
                                                                ...
                                                                if d7==d6:
                                                                   if d9.isdigit()==False:
                                                                      if d10.isdigit()==True:
                                                                         ...
                                                                         if ans=='yes':
                                                                            dot2=Canvas(root1,bg="white",width=1366,height=768)
                                                                            dot2.place(x=0,y=0)
                                                                            my1=PhotoImage(file='{}/bbg6.png'.format(path))
                                                                        ...
                                                                        #========================================                
                                                                            ...
                                                                            my2=PhotoImage(file='{}/newlogo.png'.format(path))
                                                                            ...
                                                                            def r(x=0):
                                                                                def bacteria():
                                                                                    ...
                                                                                    k.write('{}/'.format(path)+rt+'.gif\n')
                                                                                    k.write('{}/'.format(path)+rt+'.png')
                                                                                    k.write('\n--------------------------------------\n')
                                                                                    k.close()
                                                                                    corona()
                                                                                def virus():
                                                                                ...
                                                                                global my_image1
                                                                                try:
                                                                                    my_image1=PhotoImage(file='{}/'.format(path)+rt+'.gif')
                                                                                    my_image11=my_image1.subsample(2,2)
                                                                                    dot1.create_image(13, 10, anchor = NW, image=my_image11)
                                                                                except:
                                                                                    pass
                                                                                try:
                                                                                    my_image2=PhotoImage(file='{}/'.format(path)+rt+'.png')
        ...
        def b_11():
            ...
            try:
                ...
                immg=PhotoImage(file="{}/pr.png".format(path))
                ...
                img1g=PhotoImage(file="{}/bck.png".format(path))
    ...
    def sps(x=0):
        ...
        s1.place(x=0,y=0)
        my_image4=PhotoImage(file='{}/sc6.png'.format(path))
        s1.create_image(650, 0, anchor = N, image=my_image4)
        my_image5=PhotoImage(file='{}/bb12.png'.format(path))
        s1.create_image(500, 180, anchor = N, image=my_image5)
        my_image6=PhotoImage(file='{}/bb12.png'.format(path))
        s1.create_image(180, 135, anchor = N, image=my_image6)
        my_image7=PhotoImage(file='{}/bb6.png'.format(path))
        s1.create_image(170, 280, anchor = N, image=my_image7)
        my_image8=PhotoImage(file='{}/bb12.png'.format(path))
        s1.create_image(820, 330, anchor = N, image=my_image8)
        my_image9=PhotoImage(file='{}/bb11.png'.format(path))
        s1.create_image(1170, 320, anchor = N, image=my_image9)
        my_image10=PhotoImage(file='{}/bb13.png'.format(path))
        s1.create_image(680,0, anchor = N, image=my_image10)
        image=PhotoImage(file='{}/mbsa5.png'.format(path))
        image1=image.subsample(2,2)
        image2=PhotoImage(file='{}/mbsa6.png'.format(path))
...
#$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$44
        def dt():
            s3=Canvas(root1,bg="white",width=1366,height=768)
            s3.place(x=0,y=0)
            my_image11=PhotoImage(file='{}/bbg4.png'.format(path))
            s3.create_image(685, 0, anchor =N, image=my_image11)
            #enter details label======================================================
        ...
        #=========================-=-=-=-=-=-=-
            ...
            B11=Button(s3,text="NEXT",command=B_11,bg="cyan",width=14,fg='red',relief=RAISED)
            B11.place(x=1000,y=610)
            B11.config(font=("Courier",16))
            B11.bind("<Enter>",B_11)
...
s=Button(s,text='START',command=pro,bg='green')
s.place(x=640,y=250)
